...
public class Reader implements Sink {
    ...
    private static int nodeNum, wayNum;
    ...
    private boolean isAppropriate(Way way) {
        ...
        for (Tag tag : way.getTags()) {
            if (tag.getKey().equals("highway") ) {
                carAllowed =  !filterTags.contains(tag.getValue());
            }
        }
        return carAllowed;
    }
     ...
     *  src: <a href="https://wiki.openstreetmap.org/wiki/Key:highway">...</a>
     *
     * I pooled out "trunk".
     */
    private final HashSet<String> filterTags = new HashSet<>(Arrays.asList( "motorway", "pedestrian", "footway", "bridleway", "steps", "path", "cycleway",
